...
AWS_REGION=$AWS_REGION helm repo add stable-pet-clinic s3://petclinic-helm-charts-jelloul/stablemyapp/ || echo "repository name already exists"
AWS_REGION=$AWS_REGION helm repo update
helm package k8s/petclinic_chart
AWS_REGION=$AWS_REGION helm s3 push --force petclinic_chart-${BUILD_NUMBER}.tgz stable-pet-clinic
